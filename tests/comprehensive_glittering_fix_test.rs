...
mod comprehensive_glittering_fix_tests {
    use maplet::{
        core::{geo::LatLng, viewport::Viewport, geo::Point, map::Map},
    ...
    fn test_viewport_change_detection_logic() {
        ...
        let size3 = Point::new(810.0, 610.0); // Different size
         ...
         let center_diff_1_2: f64 = (center1.lat - center2.lat).abs() + (center1.lng - center2.lng).abs();
         let center_diff_1_3: f64 = (center1.lat - center3.lat).abs() + (center1.lng - center3.lng).abs();
         
         assert!(center_diff_1_2 <= 0.001, "Same centers should have minimal difference");
         assert!(center_diff_1_3 > 0.001, "Different centers should exceed threshold");
         ...
         assert!(zoom_diff_1_2 <= 0.1, "Same zoom should have minimal difference");
         assert!(zoom_diff_1_3 > 0.1, "Different zoom should exceed threshold");
        ...
        assert!(size_diff_1_2 <= 10.0, "Same size should have minimal difference");
        assert!(size_diff_1_3 > 10.0, "Different size should exceed threshold");
    }
    ...
    fn test_render_throttling_intervals() {
        ...
        assert!(core_map_interval >= 100, "Core map should have minimum 100ms interval");
        
        // Tile layer: 200ms minimum
        let tile_layer_interval = 200u64; 
        assert!(tile_layer_interval >= 200, "Tile layer should have minimum 200ms interval");
        
        // Widget expensive ops: 500ms minimum
        let widget_interval = 500u64;
        assert!(widget_interval >= 500, "Widget expensive ops should have minimum 500ms interval");
        
        // Hierarchy should be: widget >= tile layer >= core map
        assert!(widget_interval >= tile_layer_interval, "Widget interval should be >= tile layer interval");
        assert!(tile_layer_interval >= core_map_interval, "Tile layer interval should be >= core map interval");
    }
    ...
    fn test_texture_name_stability() {
        ...
        assert_eq!(hash1, hash2, "Identical tile data should produce identical texture hashes");
        
        let texture_name1 = format!("maplet_tile_cached_{:016x}", hash1);
        let texture_name2 = format!("maplet_tile_cached_{:016x}", hash2);
        
        assert_eq!(texture_name1, texture_name2, "Texture names should be stable");
    }
    ...
    fn test_needs_repaint_logic() {
        ...
        assert!(!loading_state_changed, "Fresh state should not need repaint");
        
        // Loading state changed should trigger repaint
        let loading_state_changed = true;
        assert!(loading_state_changed, "Loading state change should trigger repaint");
    }
    ...
    fn test_viewport_change_thresholds() {
        ...
        assert!(center_threshold >= 0.0001 && center_threshold <= 0.01, "Center threshold should be reasonable");
        assert!(zoom_threshold >= 0.05 && zoom_threshold <= 0.5, "Zoom threshold should be reasonable");
        assert!(size_threshold >= 5.0 && size_threshold <= 50.0, "Size threshold should be reasonable");
    }
} 