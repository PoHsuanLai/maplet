...
mod animation_loop_tests {
    ...
    fn test_no_infinite_animation_loop() {
        ...
        assert!(tiny_center_diff < center_threshold, "Tiny center differences should not trigger updates");
        assert!(tiny_zoom_diff < zoom_threshold, "Tiny zoom differences should not trigger updates");
        ...
        assert!(significant_center_diff > center_threshold, "Significant center differences should trigger updates"); 
        assert!(significant_zoom_diff > zoom_threshold, "Significant zoom differences should trigger updates");
    }
    ...
    fn test_view_update_thresholds() {
        ...
        assert!(small_center_diff < center_diff_threshold, "Small center changes should be ignored");
        assert!(small_zoom_diff < zoom_diff_threshold, "Small zoom changes should be ignored");
        
        // Large differences should trigger updates
        let large_center_diff = 0.02;
        let large_zoom_diff = 1.0;
        
        assert!(large_center_diff > center_diff_threshold, "Large center changes should trigger updates");
        assert!(large_zoom_diff > zoom_diff_threshold, "Large zoom changes should trigger updates");
    }
    ...
    fn test_conservative_frame_interval() {
        ...
        assert!(min_interval_ms >= 200, "Frame interval should be at least 200ms to prevent glittering");
        
        // Calculate max FPS from interval
        let max_fps = 1000.0 / min_interval_ms as f64;
        assert!(max_fps <= 5.0, "Max FPS should be 5 or less for tile updates");
    }
} 