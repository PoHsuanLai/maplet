...
use crate::{
    ...
    core::{
        config::{MapPerformanceProfile, MapPerformanceOptions},
        geo::{LatLng, Point},
        map::{Map, MapOptions},
        viewport::Viewport,
    },
...
impl MapBuilder {
    ...
    pub fn with_zoom_limits(mut self, min_zoom: Option<f64>, max_zoom: Option<f64>) -> Self {
        self.map_options.min_zoom = min_zoom;
        self.map_options.max_zoom = max_zoom;
        self
    }
    ...
    pub fn build(self) -> Result<Map> {
        ...
        let viewport = self.viewport.ok_or_else(|| {
            crate::Error::InvalidCoordinates("No viewport specified".to_string())
        })?;
        ...
        let task_config = self.task_config.unwrap_or_else(|| {
            TaskManagerConfig {
                max_concurrent_tasks: performance_options.tile_loader.recommended_concurrent_tasks(),
        ...
        let mut map = Map::with_options_and_performance(viewport, self.map_options, performance_options, task_config)?;
...
mod tests {
    ...
    fn test_map_builder_basic() {
        ...
        assert!(matches!(builder.performance, MapPerformanceProfile::Balanced));
    }
    ...
    fn test_mobile_map_preset() {
        ...
        assert!(builder.map_options.touch_zoom);
        assert!(!builder.map_options.keyboard);
        assert!(matches!(builder.performance, MapPerformanceProfile::LowQuality));
    }
    ...
    fn test_custom_performance_options() {
        ...
        let builder = MapBuilder::new()
            .with_performance_options(custom_options.clone());
