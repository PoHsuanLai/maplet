...
use std::sync::Arc;
use crate::background::tasks::{BackgroundTask, TaskPriority, TaskId};
...
impl BackgroundTask for GeoJsonParseTask {
    fn execute(&self) -> Pin<Box<dyn Future<Output = Result<Box<dyn std::any::Any + Send>>> + Send + '_>> {
        Box::pin(async move {
            ...
            let source = self.source_url.clone().unwrap_or_else(|| "inline".to_string());
            ...
            let result = tokio::task::spawn_blocking(move || {
                ...
                match serde_json::from_str::<GeoJson>(&data) {
                    ...
                    Err(e) => {
                        ...
                        Err(Box::new(crate::Error::ParseError(format!("GeoJSON parse error: {}", e))) as Box<dyn std::error::Error + Send + Sync>)
                    }
                }
            }).await;
            ...
            match result {
                ...
                Err(e) => {
                    ...
                    return Err(Box::new(crate::Error::Plugin(format!("Background task error: {}", e))) as Box<dyn std::error::Error + Send + Sync>);
                }
            }
        })
    }
...
impl BackgroundTask for GeoJsonLoadTask {
    fn execute(&self) -> Pin<Box<dyn Future<Output = Result<Box<dyn std::any::Any + Send>>> + Send + '_>> {
        Box::pin(async move {
            #[cfg(feature = "debug")]
            info!("Loading GeoJSON from URL: {} (task: {})", self.url, self.task_id);
                ...
                Ok(response) => {
                    match response.text().await {
                        ...
                        Err(e) => {
                            ...
                            Err(Box::new(crate::Error::Network(e)) as Box<dyn std::error::Error + Send + Sync>)
                        }
                    }
                }
                Err(e) => {
            ...
            let result = tokio::task::spawn_blocking(move || {
                ...
                for feature in &features {
                    if let Some(geometry) = &feature.geometry {
                        if let Some(geom_bounds) = crate::data::geojson::GeoJsonLayer::geometry_bounds(geometry) {
                            ...
                            if filter_expr.contains('=') {
                                ...
                                if parts.len() == 2 {
                                    if let Some(value) = properties.get(parts[0]) {
                                        return value.as_str().map_or(false, |v| v == parts[1]);
                                    }
                                }
                            }
                        }
...
pub mod tasks {
    ...
    pub fn filter_features(task_id: String, features: Vec<GeoJsonFeature>, filter_expression: String) -> FilterFeaturesTask {
